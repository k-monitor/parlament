...
async function toggle() {
  ...
  try {
    ...
    text.value = (res.sentences || []).map((s) => s.text).join(' ')
    loaded.value = true
  } catch {
    error.value = true
  } finally {
...
<template>
  <li class="speech-item" :class="{ procedural: speech.procedural, open }">
    ...
    <div v-if="open" class="speech-body">
      ...
      <p v-else class="transcript-text">{{ text }}</p>
    </div>
  </li>
</template>
...
.speech-item.open { background: var(--accent-soft); }
...
.speech-body { padding: 0 .9rem .8rem calc(.7rem + 48px + .6rem); animation: reveal .13s ease-out; }
.transcript-text { margin: 0; color: var(--ink-soft); line-height: 1.6; white-space: pre-wrap; }
